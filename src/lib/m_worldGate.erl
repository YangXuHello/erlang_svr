%%%-----------------------------------
%%% @Module  : m_login
%%% @Description: 用户 enter game,leave game.
%%%-----------------------------------
-module(m_worldGate).
-export([enterWorld/3, leaveWorld/1, fix_pid/1, stop_all/0]).
-include("common.hrl").
-include("record.hrl").

%%用户登陆
enterWorld(Player , check, Socket)  ->
    [Id | _] = Player,
    %% 检查死进程
    Pid1 = none, %%暂时默认
    Pid2 = fix_pid(Pid1),
    ...
    {Pid3, Result} = enterWorld(Pid2, Condition, Socket),
    Time = util:unixtime()+5,
    %%更新
    case db_sql:execute(io_lib:format(<<"UPDATE `player` SET `last_login_time` = ~p, 
            `online_flag`=1 where id=~p">>,[Time, Id])) of
        1 ->
                %%登陆启动
...
enterWorld(start, [Id, Accname], Socket) ->
    case m_account:get_info_by_id(Id) of
        [] ->
            {error, fail};
        Player ->
            [_, _, Aname | _] = Player,
...
%%重新登陆
enterWorld(Pid, player_online, Socket) ->
    %通知客户端账户在别处登陆
    %{ok, BinData} = pt_10:write(10007, []),
    ?INFO("notify_sec_loginin"),
    BinData = iolist_to_binary(login_pb:encode_notify_sec_loginin(#notify_sec_loginin{})),
    m_send:send_one_msg(Socket,BinData,notify_sec_loginin);
    gen_server:cast(Pid, {'SEND',BinData}),
    leaveWorld(Pid),
    enterWorld(Pid, player_offline, Socket);

%% 开始一个游戏进程
enterWorld(_Pid, player_offline, _Socket) ->
    {ok, Pid} = m_player:start(),
    {Pid, {ok, Pid}};

enterWorld(_Player, _S, _Arg) ->
    {error, fail}.
...
do_stop_all([]) ->
    ok;
do_stop_all([H | T]) ->
    leaveWorld(H#ets_online.pid),
    do_stop_all(T).


%%退出登陆
leaveWorld(Pid) when is_pid(Pid) ->
    mod_player:stop(Pid),
    ok;
leaveWorld(Status) ->
