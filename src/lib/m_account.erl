...
%% 通过帐号名称取得帐号信息
get_info_by_id(Pid) ->
    db_sql:get_row(io_lib:format(<<"SELECT id, accid, accname, nickname, scene, x, y, sex, career, lv, hp, hp_lim, mp, mp_lim, att, def, hit, dodge, crit, ten, coin, bcoin, cell_num, mount, exp, guild_id, guild_name, guild_position, gold, silver, forza, agile, wit, realm, spirit, att_area, speed, att_speed, equip, 
    		quickbar, online_flag, pet_upgrade_que_num FROM `player` WHERE id=~p limit 1">>, [Pid])).

%% 取得指定帐号的角色列表
get_role_list(Name) ->
    Sql = lists:concat(["SELECT id, status, nickname,career FROM player WHERE accname='",Name,"'"]),
    db_sql:get_all(Sql).
...
% %% 创建角色
create_role(AccId, AccName, Name, Career) ->
    % 职业
    ...
    % Sql = db_sql:make_insert_sql(player, ["accid", "accname", "career", "realm", "sex", 
    % 		"nickname", "reg_time", "last_login_time", "x", "y", "scene", "forza", "agile", "wit", "coin", "silver", 
    % 		"gold", "hp", "mp", "spirit","att_area", "att_speed", "speed", "cell_num"],
    %     [AccId, AccName, Career1, Realm1, Sex1, Name, Time, Time, X, Y, SceneId, Forza, Agile, Wit, Coin, Silver, 
    %     	Gold, Hp, Mp, Spirit, AttArea, AttSpeed, Speed, CellNum]),


    Sql = db_sql:make_insert_sql(player, ["accid", "accname", "career",
    		"nickname", "reg_time", "last_login_time", "x", "y", "scene", 
    		"hp", "mp"],
        [AccId, AccName, Career1, Name, Time, Time, X, Y, SceneId, 100, 100]),
