...
send_one_msg(S,BinData,MsgType) ->
    BinHeader = iolist_to_binary(header_pb:encode_header(#header{
        type = msg_trans:enum_to_int(msgtype,MsgType),
        size = byte_size(BinData)})),
                                                        
    BinSend = <<BinHeader/binary,BinData/binary>>,                                                        
    m_send:send_one(S, BinSend).
